...
export default function SpotClientPage({ initialSpot, initialRandomSpots }: SpotClientPageProps) {
  ...
  useEffect(() => {
    async function initPage() {
      ...
      if (spot) {
        // Galerie aus den Server-Daten parsen
        let parsedGallery = [];
        if (spot.gallery_urls) {
          try {
            parsedGallery = Array.isArray(spot.gallery_urls) ? spot.gallery_urls : JSON.parse(spot.gallery_urls);
        ...
        // User-spezifische Daten laden (Favoriten & Hotel)
        const { data: { user: userData } } = await supabase.auth.getUser();
        if (userData) {
          // Favoriten checken
          const { data: fav } = await supabase.from("favorites").select("id").eq("spot_id", spot.id).eq("user_id", userData.id).maybeSingle();
          ...
          if (hLat && hLng) {
            fetchDrivingDistance({lat: hLat, lng: hLng}, {lat: spot.latitude, lng: spot.longitude});
          }
        }
      }
    }
            ...
            <div style={{ background: "#ffffff", borderRadius: 24, padding: "32px", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.05)", border: "1px solid #f1f5f9" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24, marginBottom: 40 }}>
                ...
                <InfoItem 
                    ...
                    label="Fahrtweg" 
                    value={isRouting ? "..." : routeDist ? `${routeDist} km (${routeTime} Min.)` : <Link href="/profile" className="text-teal-600 underline font-bold">Hotel festlegen</Link>} 
                />
