"use client";

import type { ReactNode } from "react";
import {
  ArrowRight,
  Check,
  Clock3,
  Flower2,
  MapPin,
  Sparkles,
  WalletCards,
} from "lucide-react";
import type { Language } from "@/src/lib/i18n";
import type {
  HotelProfileRecord,
  HotelSpaRecord,
} from "@/src/types/spot";

interface HotelSpaProps {
  spaAreas: HotelSpaRecord[];
  hotelProfile: HotelProfileRecord;
  language: Language;
  userRole?: string | null;
}

const labels = {
  de: {
    eyebrow: "Spa & Wellness",
    title: "Entspannung für Körper und Geist",
    subtitle:
      "Spa-Bereiche, Behandlungen und Wellnessangebote des Resorts im Überblick.",
    location: "Lage",
    openingHours: "Öffnungszeiten",
    treatments: "Behandlungen",
    from: "ab",
    reservation: "Mehr erfahren",
    draft: "Redaktionelle Vorschau",
    draftNotice:
      "Diese Spa-Informationen sind noch nicht zur Veröffentlichung freigegeben.",
  },
  en: {
    eyebrow: "Spa & wellness",
    title: "Relaxation for body and mind",
    subtitle:
      "An overview of the resort's spa areas, treatments and wellness experiences.",
    location: "Location",
    openingHours: "Opening hours",
    treatments: "Treatments",
    from: "from",
    reservation: "Learn more",
    draft: "Editorial preview",
    draftNotice:
      "This spa information has not yet been approved for publication.",
  },
} as const;

function localizedValue(
  spa: HotelSpaRecord,
  language: Language,
  field:
    | "name"
    | "description"
    | "location"
    | "opening_hours"
): string {
  const primary =
    language === "en"
      ? spa[`${field}_en`]
      : spa[`${field}_de`];

  const fallback =
    language === "en"
      ? spa[`${field}_de`]
      : spa[`${field}_en`];

  if (
    typeof primary === "string" &&
    primary.trim() !== ""
  ) {
    return primary.trim();
  }

  if (
    typeof fallback === "string" &&
    fallback.trim() !== ""
  ) {
    return fallback.trim();
  }

  return "";
}

function localizedList(
  primary: unknown,
  fallback: unknown
): string[] {
  const source = Array.isArray(primary)
    ? primary
    : Array.isArray(fallback)
      ? fallback
      : [];

  return source.filter(
    (item): item is string =>
      typeof item === "string" &&
      item.trim() !== ""
  );
}

function formatPrice(
  price: number,
  currency?: string | null
): string {
  const normalizedCurrency =
    currency?.trim().toUpperCase() || "THB";

  try {
    return new Intl.NumberFormat("de-DE", {
      style: "currency",
      currency: normalizedCurrency,
      maximumFractionDigits: 0,
    }).format(price);
  } catch {
    return `${price} ${normalizedCurrency}`;
  }
}

export default function HotelSpa({
  spaAreas,
  hotelProfile,
  language,
  userRole,
}: HotelSpaProps) {
  const copy = labels[language];

  const normalizedRole = String(userRole || "")
    .trim()
    .toLowerCase();

  const isEditor =
    normalizedRole === "admin" ||
    normalizedRole === "editor";

  const isPublished =
    hotelProfile.status === "published";

  if (!isPublished && !isEditor) {
    return null;
  }

  if (spaAreas.length === 0) {
    return null;
  }

  return (
    <section
      style={{
        maxWidth: 1180,
        margin: "0 auto 48px",
        padding: "0 28px",
      }}
    >
      <div
        style={{
          marginBottom: 26,
          display: "flex",
          alignItems: "flex-end",
          justifyContent: "space-between",
          gap: 24,
          flexWrap: "wrap",
        }}
      >
        <div style={{ maxWidth: 760 }}>
          <div
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 8,
              marginBottom: 10,
              color: "#0f766e",
              fontSize: 11,
              fontWeight: 800,
              textTransform: "uppercase",
              letterSpacing: "0.13em",
            }}
          >
            <Flower2 size={17} />
            {copy.eyebrow}
          </div>

          <h2
            style={{
              margin: 0,
              color: "#0f172a",
              fontSize: "clamp(28px, 4vw, 42px)",
              lineHeight: 1.12,
              fontWeight: 800,
              letterSpacing: "-0.04em",
            }}
          >
            {copy.title}
          </h2>

          <p
            style={{
              margin: "12px 0 0",
              maxWidth: 720,
              color: "#64748b",
              fontSize: 15,
              lineHeight: 1.75,
            }}
          >
            {copy.subtitle}
          </p>
        </div>

        {!isPublished && isEditor && (
          <div
            style={{
              maxWidth: 290,
              padding: "12px 14px",
              borderRadius: 16,
              background: "#fff7ed",
              border: "1px solid #fed7aa",
              color: "#9a3412",
            }}
          >
            <div
              style={{
                marginBottom: 4,
                fontSize: 10,
                fontWeight: 800,
                textTransform: "uppercase",
                letterSpacing: "0.08em",
              }}
            >
              {copy.draft}
            </div>

            <div
              style={{
                fontSize: 11,
                fontWeight: 600,
                lineHeight: 1.5,
              }}
            >
              {copy.draftNotice}
            </div>
          </div>
        )}
      </div>

      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            "repeat(auto-fit, minmax(300px, 1fr))",
          gap: 18,
        }}
      >
        {spaAreas.map((spa) => {
          const name = localizedValue(
            spa,
            language,
            "name"
          );

          const description = localizedValue(
            spa,
            language,
            "description"
          );

          const location = localizedValue(
            spa,
            language,
            "location"
          );

          const openingHours = localizedValue(
            spa,
            language,
            "opening_hours"
          );

          const treatments = localizedList(
            language === "en"
              ? spa.treatments_en
              : spa.treatments_de,
            language === "en"
              ? spa.treatments_de
              : spa.treatments_en
          );

          const highlights = localizedList(
            language === "en"
              ? spa.highlights_en
              : spa.highlights_de,
            language === "en"
              ? spa.highlights_de
              : spa.highlights_en
          );

          return (
            <article
              key={spa.id}
              style={{
                overflow: "hidden",
                borderRadius: 24,
                background: "#ffffff",
                border: "1px solid #e2e8f0",
                boxShadow:
                  "0 16px 40px rgba(15,23,42,0.08)",
              }}
            >
              {spa.image_url && (
                <div
                  style={{
                    height: 220,
                    overflow: "hidden",
                    background: "#e2e8f0",
                  }}
                >
                  <img
                    src={spa.image_url}
                    alt={name}
                    style={{
                      width: "100%",
                      height: "100%",
                      objectFit: "cover",
                    }}
                  />
                </div>
              )}

              <div style={{ padding: 24 }}>
                <div
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 7,
                    marginBottom: 11,
                    color: "#0f766e",
                    fontSize: 10,
                    fontWeight: 800,
                    textTransform: "uppercase",
                    letterSpacing: "0.08em",
                  }}
                >
                  <Sparkles size={16} />
                  {copy.eyebrow}
                </div>

                <h3
                  style={{
                    margin: 0,
                    color: "#0f172a",
                    fontSize: 22,
                    lineHeight: 1.2,
                    fontWeight: 800,
                    letterSpacing: "-0.03em",
                  }}
                >
                  {name}
                </h3>

                {description && (
                  <p
                    style={{
                      margin: "12px 0 0",
                      color: "#64748b",
                      fontSize: 13,
                      lineHeight: 1.7,
                    }}
                  >
                    {description}
                  </p>
                )}

                {(location || openingHours) && (
                  <div
                    style={{
                      display: "grid",
                      gap: 10,
                      marginTop: 20,
                    }}
                  >
                    {location && (
                      <SpaFact
                        icon={<MapPin size={16} />}
                        label={copy.location}
                        value={location}
                      />
                    )}

                    {openingHours && (
                      <SpaFact
                        icon={<Clock3 size={16} />}
                        label={copy.openingHours}
                        value={openingHours}
                      />
                    )}
                  </div>
                )}

                {treatments.length > 0 && (
                  <div style={{ marginTop: 20 }}>
                    <div
                      style={{
                        marginBottom: 10,
                        color: "#64748b",
                        fontSize: 9,
                        fontWeight: 800,
                        textTransform: "uppercase",
                        letterSpacing: "0.08em",
                      }}
                    >
                      {copy.treatments}
                    </div>

                    <div
                      style={{
                        display: "flex",
                        flexWrap: "wrap",
                        gap: 8,
                      }}
                    >
                      {treatments.map((treatment) => (
                        <span
                          key={treatment}
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: 6,
                            padding: "7px 10px",
                            borderRadius: 999,
                            background: "#f5f3ff",
                            color: "#6d28d9",
                            fontSize: 10,
                            fontWeight: 700,
                          }}
                        >
                          <Flower2 size={13} />
                          {treatment}
                        </span>
                      ))}
                    </div>
                  </div>
                )}

                {highlights.length > 0 && (
                  <div
                    style={{
                      display: "flex",
                      flexWrap: "wrap",
                      gap: 8,
                      marginTop: 18,
                    }}
                  >
                    {highlights.map((highlight) => (
                      <span
                        key={highlight}
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: 6,
                          padding: "7px 10px",
                          borderRadius: 999,
                          background: "#f0fdfa",
                          color: "#115e59",
                          fontSize: 10,
                          fontWeight: 700,
                        }}
                      >
                        <Check size={13} />
                        {highlight}
                      </span>
                    ))}
                  </div>
                )}

                {(spa.price_from ||
                  spa.reservation_url) && (
                  <div
                    style={{
                      marginTop: 22,
                      paddingTop: 18,
                      borderTop: "1px solid #e2e8f0",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                      gap: 16,
                    }}
                  >
                    {spa.price_from ? (
                      <div>
                        <div
                          style={{
                            color: "#64748b",
                            fontSize: 10,
                            fontWeight: 700,
                          }}
                        >
                          {copy.from}
                        </div>

                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: 7,
                            color: "#0f172a",
                            fontSize: 19,
                            fontWeight: 800,
                          }}
                        >
                          <WalletCards
                            size={18}
                            color="#0f766e"
                          />
                          {formatPrice(
                            spa.price_from,
                            spa.currency
                          )}
                        </div>
                      </div>
                    ) : (
                      <div />
                    )}

                    {spa.reservation_url && (
                      <a
                        href={spa.reservation_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: 7,
                          padding: "10px 13px",
                          borderRadius: 12,
                          background: "#0f766e",
                          color: "#ffffff",
                          textDecoration: "none",
                          fontSize: 11,
                          fontWeight: 800,
                        }}
                      >
                        {copy.reservation}
                        <ArrowRight size={15} />
                      </a>
                    )}
                  </div>
                )}
              </div>
            </article>
          );
        })}
      </div>
    </section>
  );
}

function SpaFact({
  icon,
  label,
  value,
}: {
  icon: ReactNode;
  label: string;
  value: string;
}) {
  return (
    <div
      style={{
        display: "flex",
        alignItems: "flex-start",
        gap: 10,
        padding: "11px 12px",
        borderRadius: 14,
        background: "#f8fafc",
        border: "1px solid #f1f5f9",
      }}
    >
      <div
        style={{
          color: "#0f766e",
          marginTop: 1,
        }}
      >
        {icon}
      </div>

      <div>
        <div
          style={{
            color: "#64748b",
            fontSize: 9,
            fontWeight: 800,
            textTransform: "uppercase",
            letterSpacing: "0.07em",
            marginBottom: 3,
          }}
        >
          {label}
        </div>

        <div
          style={{
            color: "#334155",
            fontSize: 11,
            fontWeight: 700,
            lineHeight: 1.45,
          }}
        >
          {value}
        </div>
      </div>
    </div>
  );
}
